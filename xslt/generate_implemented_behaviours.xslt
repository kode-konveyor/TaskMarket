...
<xsl:stylesheet version="2.0"
		...
		<features>
			<xsl:for-each
				...
				<feature>
                    ...
                    <xsl:variable name="milestone" select="zenta:neighbours($rich,$step,'implements,2')"/>
	...
	<xsl:template match="/">
		...
		<xsl:result-document
			href="shippable/implementedTestCases.xml">
			<xsl:copy-of select="$testcases" />
		</xsl:result-document>
		<xsl:result-document
			href="shippable/implementedBehaviours.xml">
			<xsl:copy-of select="$behaviours" />
		</xsl:result-document>
		<xsl:result-document
			href="shippable/implementedBehaviours.docbook">
